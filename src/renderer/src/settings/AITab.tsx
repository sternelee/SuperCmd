...
const AITab: React.FC = () => {
  ...
  useEffect(() => {
    if (!settings || settings.ai.provider !== 'ollama' || !settings.ai.enabled) return;
    refreshOllamaStatus();
  }, [settings?.ai?.provider, settings?.ai?.enabled, settings?.ai?.ollamaBaseUrl, refreshOllamaStatus]);
